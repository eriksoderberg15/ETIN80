...
#include "ETIN80_group_7_h.h" 
    ...
    */
          ...
          //  else  temp_drive = (float)input.right * 100;
    		...
    		if(temp_drive > 0.666) overdrive_out_sample = 1;
    		else if(temp_drive <= 0.666 && temp_drive > 0.333) overdrive_out_sample = 0.333*(3-(2-3*temp_drive)*(2-3*temp_drive));	
    		
    		else if(temp_drive <= 0.333 && temp_drive > 0.00007) overdrive_out_sample = 2*temp_drive;//[-0.0001,0,0001]
    		
    		else if(temp_drive <= 0.00007 && temp_drive > -0.00007) overdrive_out_sample = 0;
    		
    		else if(temp_drive >= -0.333 && temp_drive <= -0.00007) overdrive_out_sample = -2*temp_drive;
    		
    		else if(temp_drive >= -0.666 && temp_drive < -0.333) overdrive_out_sample = 0.333*((2+3*temp_drive)*(2+3*temp_drive) - 3);
